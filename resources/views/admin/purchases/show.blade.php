...
@section('title', 'Detalle de Compra - Administrador')
...
<div class="max-w-4xl space-y-6">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <div class="flex items-center justify-between mb-6">
            <h3 class="text-xl font-semibold text-gray-800">Información de la Compra</h3>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <p class="text-lg font-semibold text-gray-900">{{ $purchase->peasant->name }}</p>
                <p class="text-sm text-gray-600 mt-1">{{ $purchase->peasant->email }}</p>
            </div>
            ...
            @if($purchase->observations)
                <div class="md:col-span-2">
                    ...
                    <p class="text-gray-900 bg-gray-50 p-3 rounded-lg">{{ $purchase->observations }}</p>
                </div>
            @endif
        </div>
    </div>
    ...
    @if($purchase->payments->count() > 0)
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            <h3 class="text-lg font-semibold text-gray-800 mb-4">Pagos Realizados</h3>
            <div class="space-y-3">
                @foreach($purchase->payments as $payment)
                    <div class="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                        ...
                        <span class="px-3 py-1 text-xs font-semibold rounded-full {{ $payment->status === 'completed' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800' }}">
    ...
    @if($purchase->invoice)
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            <div class="flex items-center justify-between">
                ...
                <a href="{{ route('admin.invoices.show', $purchase->invoice) }}" 
